...
export default function TestimonialsAdmin() {
    ...
    const handleDelete = async (id) => {
        const ok = await confirm('Hapus Testimonial', 'Testimoni ini akan dihapus permanen. Lanjutkan?');
        if (!ok) return;
    ...
    const moveItem = async (id, direction) => {
        ...
        await Promise.all([
            supabase.from('testimonials').update({ display_order: idx }).eq('id', reordered[idx].id),
            supabase.from('testimonials').update({ display_order: newIdx }).eq('id', reordered[newIdx].id),
        ]);
        fetchAll();
    };
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-[40px_1fr_auto] gap-6 items-start">
                                    ...
                                    <div className="space-y-5">
                                        ...
                                        <div className="border-b border-white/[0.05] pb-4 focus-within:border-[#CEB175]/25 transition-colors">
                                            <p className="text-[7px] uppercase tracking-[0.4em] text-white/25 mb-2">Kutipan</p>
                                            <textarea
                                                rows={3}
                                                defaultValue={item.quote || ''}
                                                onBlur={e => handleField(item.id, 'quote', e.target.value)}
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            ...
                                            <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                                <p className="text-[7px] uppercase tracking-[0.4em] text-white/25 mb-2">Inisial</p>
                                                <input
                                                    type="text"
                                                    defaultValue={item.initials || ''}
                                                    onBlur={e => handleField(item.id, 'initials', e.target.value)}
                                                    placeholder="R & D"
                                                    className="w-full bg-transparent text-sm text-[#CEB175] font-serif italic focus:outline-none placeholder:text-white/10"
                                                />
                                                <p className="text-[7px] text-white/15 mt-1">Gunakan format &quot;A & B&quot;</p>
                                            </div>

                                            {/* Year */}
                                            <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                                <p className="text-[7px] uppercase tracking-[0.4em] text-white/25 mb-2">Tahun</p>
                                                <input
                                                    type="text"
                                                    defaultValue={item.year || ''}
                                                    onBlur={e => handleField(item.id, 'year', e.target.value)}
                                                    placeholder="2024"
                                            ...
                                            {/* Venue hint */}
                                            <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                                <p className="text-[7px] uppercase tracking-[0.4em] text-white/25 mb-2">Lokasi (Cryptic)</p>
                                                <input
                                                    type="text"
                                                    defaultValue={item.venue_hint || ''}
                                                    onBlur={e => handleField(item.id, 'venue_hint', e.target.value)}
                                        ...
                                        <div className="flex items-center gap-6 pt-1">
                                            <button
                                                onClick={() => handleField(item.id, 'is_active', !item.is_active)}
