...
export default function VenuesAdmin() {
    ...
    const handleField = async (id, field, value) => {
        ...
        await supabase.from('venues').update({ [field]: value }).eq('id', id);
        setVenues((p) => p.map((v) => (v.id === id ? { ...v, [field]: value } : v)));
        setSaving((p) => ({ ...p, [id]: false }));
    };

    const handleDelete = async (id) => {
        const ok = await confirm('Hapus Venue', 'Venue ini akan dihapus permanen. Lanjutkan?');
        if (!ok) return;
    ...
    const moveItem = async (id, direction) => {
        ...
        await Promise.all([
            supabase.from('venues').update({ display_order: idx }).eq('id', reordered[idx].id),
            supabase.from('venues').update({ display_order: newIdx }).eq('id', reordered[newIdx].id),
        ]);
        fetchAll();
    };
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-10 pb-6 border-b border-white/[0.06]">
                <p className="text-[10px] uppercase tracking-[0.5em] text-white/40 font-light">
                    {loading ? '— memuat —' : `${venues.length} Venue${venues.length !== 1 ? 's' : ''}`}
                </p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-[100px_1fr_auto] gap-6 items-start">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                            <p className="text-[7px] uppercase tracking-[0.4em] text-white/25 mb-2">Nama Venue</p>
                                            <input
                                                type="text"
                                                defaultValue={v.name || ''}
                                                onBlur={(e) => handleField(v.id, 'name', e.target.value)}
                                        ...
                                        <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                            ...
                                            <textarea
                                                ...
                                                onBlur={(e) => handleField(v.id, 'cryptic_caption', e.target.value)}
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                                ...
                                                <input
                                                    ...
                                                    onBlur={(e) => handleField(v.id, 'location_hint', e.target.value)}
                                            ...
                                            <div className="border-b border-white/[0.05] pb-3 focus-within:border-[#CEB175]/25 transition-colors">
                                                ...
                                                <input
                                                    ...
                                                    onBlur={(e) => handleField(v.id, 'image_url', e.target.value)}
                                                    placeholder="/images/..."
                                                    className="w-full bg-transparent text-sm text-white focus:outline-none placeholder:text-white/10"
                                                />
                                            </div>
                                        ...
                                        <div className="flex items-center gap-6 pt-2">
                                            <button
                                                onClick={() => handleField(v.id, 'is_active', !v.is_active)}
