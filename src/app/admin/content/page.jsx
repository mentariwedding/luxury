...
export default function ContentManagement() {
  ...
  useEffect(() => {
    if (active) fetchItems(active.section_name);
  }, [active, fetchItems]);
  ...
  const handleSelect = (s) => {
    setActive(s);
    setForm(s);
    setTab(s.section_name === "partners" ? "items" : "text");
  };
  const handleInput = (e) => {
  ...
  const handleSaveMain = async (e) => {
    ...
    // Hero & Video Reel support video_url
    if (
      active?.section_name === "hero" ||
      active?.section_name === "video_reel"
    ) {
    ...
    error
      ...
      : toast("Konten berhasil disimpan!", "success");
    fetchSections();
  };
    ...
    <div>
      ...
      {loading ? (
        <div className="animate-pulse space-y-4 luxury-section">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-12 bg-white/[0.03]" />
          ))}
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-8 mb-16 border-b border-white/[0.06] pb-10">
                <div className="mb-2 sm:mb-0">
                  <p className="text-[8px] uppercase tracking-[0.6em] text-[#CEB175]/50 mb-3 italic font-serif">
                    Curating Content
                  </p>
                  <h2 className="font-serif text-3xl sm:text-4xl font-light text-white tracking-tight italic">
                    {LABELS[active.section_name]}
                  </h2>
                </div>
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    {active.section_name !== "partners" && (
                      <button
                        ...
                        className={`flex items-center gap-2 px-5 py-2.5 text-[8px] uppercase tracking-[0.35em] font-medium transition-all duration-500 ${
                          tab === "text"
                            ? "bg-[#CEB175] text-black shadow-lg shadow-[#CEB175]/10"
                            : "text-white/20 hover:text-white/50"
                        }`}
                      >
                  ...
                  {/* Preview Toggle */}
                  <MinimalistTooltip
                    text={preview ? "Tutup Pratinjau" : "Lihat Pratinjau"}
                  >
                    <button
                      type="button"
                ...
                <form onSubmit={handleSaveMain} className="space-y-0">
                  <div className="space-y-8">
                    ...
                    <UnderlineInput
                      ...
                      textarea
                      rows={4}
                      placeholder="Paragraf deskripsi section..."
                    />
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-8">
                      <UnderlineInput
                        label="URL Gambar"
                    ...
                    {(active.section_name === "hero" ||
                      ...
                      <UnderlineInput
                        label={
                          active.section_name === "video_reel"
                            ? "URL Video Highlight Reel (.mp4)"
                            : "URL Video Background (Opsional, .mp4)"
                        }
                        name="video_url"
                        value={form.video_url}
                        onChange={handleInput}
                        placeholder="https://...mp4 atau /videos/highlight.mp4"
                      />
                    )}
                    {active.section_name === "video_reel" && (
                      <div className="mt-2 p-4 border border-[#CEB175]/8 bg-[#CEB175]/[0.02]">
                        <p className="text-[7px] uppercase tracking-[0.4em] text-[#CEB175]/40 mb-2 font-light">
                          Catatan
                        </p>
                        <p className="text-[10px] text-white/25 font-light leading-relaxed">
                          <span className="text-white/40">URL Gambar</span> →
                          dipakai sebagai poster/thumbnail saat video belum
                          dimuat.
                          <br />
                          <span className="text-white/40">URL Video</span> →
                          file .mp4 yang diputar autoplay & loop. Kosongkan jika
                          section ini tidak perlu video.
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          const ok = await confirm(
                            ...
                            { confirmText: "Ya, Reset", type: "info" },
                          );
                          if (ok) {
                            setForm(active);
                  ...
                  >
                    <div className="mt-10 border border-white/[0.06] overflow-hidden">
                      ...
                      <div className="p-5 sm:p-8 md:p-12 bg-[#060606] min-h-[160px] sm:min-h-[200px]">
                        ...
                        {form.title && (
                          <h2
                            className="font-serif font-light text-white leading-[0.95] mb-4 sm:mb-6"
                        ...
                        {!form.title && !form.subtitle && !form.description && (
                          <p className="text-[9px] text-white/15 italic">
                            Mulai ketik untuk melihat preview...
                          </p>
                        )}
                      </div>
                    </div>
