...
export default function Hero() {
  const [content, setContent] = useState({
    ...
    description:
      "Kami percaya setiap pernikahan adalah cerita unik yang layak dirayakan dengan sempurna.",
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-32 right-6 md:right-12 z-10 hidden md:flex flex-col items-end gap-3">
        <span className="text-[9px] uppercase tracking-[0.5em] text-[#CEB175]/50 font-light italic font-serif">
          Mentari Wedding
        </span>
        <span className="w-px h-12 bg-[#CEB175]/30"></span>
      </div>
      ...
      <motion.div
        ...
        className="relative z-10 text-center px-4 max-w-5xl mx-auto mt-20 md:mt-24 flex flex-col items-center"
      >
        <SplitText
        ...
        <h1 className="font-serif text-[28px] sm:text-4xl md:text-8xl lg:text-9xl font-light leading-[1.1] text-white mb-10 drop-shadow-2xl">
          {titleParts[0] && (
            <>
              <SplitText
                text={titleParts[0] + (titleParts[1] ? "," : "")}
                delay={300}
              />{" "}
              <br />
            </>
      ...
      <div className="absolute bottom-8 md:bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4">
        <span className="text-[10px] uppercase tracking-[0.5em] text-[#CEB175]/60 font-light">
          Scroll
        </span>
