...
const AESTHETICS = [
    { name: 'Dusty Rose & Ivory', colors: ['#E8D0C0', '#F5EDE6', '#C9A9A6', '#8B6B5D'], tag: 'Romantic · Soft · Garden' },
    { name: 'Midnight & Gold', colors: ['#1A1A2E', '#2D2B55', '#CEB175', '#E8D399'], tag: 'Bold · Timeless · Formal' },
    { name: 'Sage & Cream', colors: ['#87A878', '#C8CDA0', '#F0EAD6', '#D4C5A9'], tag: 'Natural · Earthy · Serene' },
    { name: 'Pure White', colors: ['#F8F8F6', '#EDEDE8', '#D9D9D4', '#C4C4BE'], tag: 'Classic · Timeless · Pure' },
    { name: 'Mauve & Blush', colors: ['#C9A6B5', '#E5C9D4', '#F2E2E8', '#D4A0B5'], tag: 'Feminine · Dreamy · Soft' },
    { name: 'Belum yakin', colors: [], tag: 'Kami bantu temukan gaya Anda' },
];

const GUEST_RANGES = ['< 100 tamu', '100 – 200 tamu', '200 – 400 tamu', '> 400 tamu', 'Belum tentukan'];
...
export default function InquiryForm({ isOpen, onClose }) {
    ...
    useEffect(() => {
        if (isOpen) { setStep(1); setForm({ date: '', guests: '', aesthetic: '', initials: '', message: '' }); }
    }, [isOpen]);
                    ...
                    >
                        ...
                        <div className="px-8 pt-8 pb-6 border-b border-white/[0.05]">
                            <div className="flex items-start justify-between mb-5">
                                <div>
                                    <p className="text-[8px] uppercase tracking-[0.6em] text-[#CEB175]/60 mb-2">Inquiry</p>
                                    <h2 className="font-serif text-2xl md:text-3xl font-light text-white leading-tight">
                                        Mulai{' '}
                                        <span className="italic gold-gradient-text">Cerita Anda</span>
                                    </h2>
                                </div>
                                <button
                                    onClick={onClose}
                                        ...
                                        <div className="flex items-center gap-2">
                                            <div className={`w-5 h-5 flex items-center justify-center transition-all duration-500 ${
                                                ...
                                                {step > i + 1
                                                    ...
                                                    : <span className={`text-[8px] ${step === i + 1 ? 'text-[#CEB175]' : 'text-white/20'}`}>{i + 1}</span>
                                                }
                                            </div>
                                            <span className={`text-[8px] uppercase tracking-[0.3em] transition-colors duration-500 ${
                                                step === i + 1 ? 'text-[#CEB175]' : 'text-white/20'
                                            }`}>{label}</span>
                                        </div>
                                        {i < stepLabels.length - 1 && (
                                            <div className={`flex-1 h-px mx-3 transition-colors duration-500 ${step > i + 1 ? 'bg-[#CEB175]/40' : 'bg-white/[0.06]'}`} />
                                        )}
                                    </React.Fragment>
                                ))}
                            </div>
                        ...
                        <div className="px-8 py-8 min-h-[280px]">
                            <AnimatePresence mode="wait">
                                ...
                                {step === 1 && (
                                    ...
                                    >
                                        <div>
                                            <p className="text-[8px] uppercase tracking-[0.4em] text-white/30 mb-3">Kapan hari istimewa Anda?</p>
                                            <input
                                                type="date"
                                                value={form.date}
                                        ...
                                        <div>
                                            <p className="text-[8px] uppercase tracking-[0.4em] text-white/30 mb-3">Perkiraan jumlah tamu</p>
                                ...
                                {step === 2 && (
                                    <motion.div key="step2"
                                        ...
                                        transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
                                    >
                                        <p className="text-[8px] uppercase tracking-[0.4em] text-white/30 mb-5">Estetika mana yang paling berbicara untuk Anda?</p>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            {AESTHETICS.map(a => (
                                                ...
                                                >
                                                    ...
                                                    <p className={`text-xs font-serif italic transition-colors duration-300 ${form.aesthetic === a.name ? 'text-[#CEB175]' : 'text-white/70'}`}>
                                                        {a.name}
                                                    </p>
                                                    <p className="text-[7px] uppercase tracking-[0.3em] text-white/25 mt-0.5">{a.tag}</p>
                                                </button>
                                            ))}
                                        </div>
                                    </motion.div>
                                ...
                                {step === 3 && (
                                    ...
                                    >
                                        <div>
                                            <p className="text-[8px] uppercase tracking-[0.4em] text-white/30 mb-3">Inisial Anda berdua</p>
                                            <input
                                                type="text"
                                        ...
                                        <div>
                                            <p className="text-[8px] uppercase tracking-[0.4em] text-white/30 mb-3">Ceritakan sedikit tentang hari istimewa Anda <span className="normal-case tracking-normal text-white/15">(opsional)</span></p>
                                            <textarea
                                                rows={3}
                                            ...
                                            <div className="space-y-1">
                                                {[['Tanggal', form.date || '—'], ['Tamu', form.guests || '—'], ['Estetika', form.aesthetic || '—']].map(([k, v]) => (
                                                    <div key={k} className="flex justify-between">
                                                        <span className="text-[8px] uppercase tracking-[0.3em] text-white/25">{k}</span>
                                                        <span className="text-[9px] text-white/60 font-serif italic">{v}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
