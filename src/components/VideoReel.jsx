...
export default function VideoReel({
  src: propSrc = "/videos/highlight.mp4",
  poster: propPoster = "/images/hero.JPG",
}) {
  const ref = useRef(null);
  const videoRef = useRef(null);
  const [muted, setMuted] = useState(true);
  const [videoSrc, setVideoSrc] = useState(propSrc);
  const [videoPoster, setVideoPoster] = useState(propPoster);
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        ...
        .eq("section_name", "video_reel")
        .maybeSingle();
      if (data?.video_url) setVideoSrc(data.video_url);
      if (data?.image_url) setVideoPoster(data.image_url);
    })();
  }, []);
  ...
  return (
    <section
      ref={ref}
      className="relative h-[45vh] md:h-[55vh] overflow-hidden bg-black"
    >
      ...
      <div className="absolute bottom-6 right-6 z-30">
        ...
        >
          ...
          <svg
            ...
            viewBox="0 0 24 24"
            fill="none"
            className={`text-[#CEB175]/60 group-hover:text-[#CEB175] transition-colors duration-500`}
          >
            {muted ? (
              <>
                <path
